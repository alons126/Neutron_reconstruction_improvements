...
bool clas12ana::checkGhostTrackCD(const region_part_ptr &p)
{
    ...
    */
    ...
    {
        // case 1
        if (p->getRegion() == clas12::CD && p2->getRegion() == clas12::CD && p2->sci(CTOF)->getComponent() == p->sci(CTOF)->getComponent())
            return true;
...
bool clas12ana::CDEdgeCuts(const region_part_ptr &p)
{
    ...
    if (p->par()->getCharge() != 0 && p->getRegion() == CD) // neutral particles don't follow cuts
    {
        ...
        int hit_reg_first = hp_first < -90 ? 1 : hp_first < 30 ? 2
                                             : hp_first < 150  ? 3
                                                               : 1;
        ...
        int hit_reg_last = hp_last < -90 ? 1 : hp_last < 30 ? 2
                                           : hp_last < 150  ? 3
                                                            : 1;
...
bool clas12ana::checkVertex(const region_part_ptr &p)
{
    ...
    bool in_vxvy = (p->par()->getVx() > vertex_x_cuts.at(0) && p->par()->getVx() < vertex_x_cuts.at(1)) && (p->par()->getVy() > vertex_y_cuts.at(0) && p->par()->getVy() < vertex_y_cuts.at(1));
