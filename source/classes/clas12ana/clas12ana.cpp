        ...
        */
        ...
        std::for_each(particles.begin(), particles.end(), [this, electrons_det](auto p) {
            ...
            if (p->par()->getCharge() == 0 && p->par()->getPid() != 11) {
                ...
                bool check_pid_cuts = ((f_protonpidCuts && checkProtonPidCut(p)) ||
                                       //check if in proton PID cuts or chi2pid cuts
                                       (f_pidCuts && checkPidCut(p)) ||
                                       // if proton pid cuts if off but pid cuts on just use chi2pid
                                       (!f_protonpidCuts && !f_pidCuts));
                // if no pid cuts are specified let all particles pass pid
...
void clas12ana::checkCutParameters() {
    ...
    if (current_run >= 15542) {
        if (previous_run < 15542) // default; do nothing already been set to >=15542
        {
            ...
            std::cerr << "WARNING:: Run number changed to " << current_run <<
                    ". The SF cuts are changed to reflect this new run range" << std::endl;
        }
    }
    ...
    bool d_runrange = ((current_run >= 15043 && current_run <= 15106) || (
                           current_run >= 15433 && current_run <= 15456));
    bool he_runrange = ((current_run >= 15108 && current_run <= 15164) || (
                            current_run >= 15458 && current_run <= 15490));
    ...
    bool d_runrange_prev = ((previous_run >= 15043 && previous_run <= 15106) || (
                                previous_run >= 15433 && previous_run <= 15456));
    bool he_runrange_prev = ((previous_run >= 15108 && previous_run <= 15164) || (
                                 previous_run >= 15458 && previous_run <= 15490));
    ...
    if (he_runrange && !he_runrange_prev) {
        std::cerr << "WARNING:: Run range changed for run " << current_run << ". Setting ana_he4.par file." <<
                std::endl;
        this->readInputParam((std::string(CLAS12ANA_DIR) + "/Ana/cutFiles/ana_he4.par").c_str());
    } else if (ca40_runrange && !ca40_runrange_prev) {
        std::cerr << "WARNING:: Run range changed for run " << current_run << ". Setting ana_ca40.par file." <<
                std::endl;
        this->readInputParam((std::string(CLAS12ANA_DIR) + "/Ana/cutFiles/ana_ca40.par").c_str());
    } else if (ca48_runrange && !ca48_runrange_prev) {
        std::cerr << "WARNING:: Run range changed for run " << current_run << ". Setting ana_ca48.par file." <<
                std::endl;
        this->readInputParam((std::string(CLAS12ANA_DIR) + "/Ana/cutFiles/ana_ca48.par").c_str());
    } else if (cx4_runrange && !cx4_runrange_prev) {
        std::cerr << "WARNING:: Run range changed for run " << current_run << ". Setting ana_cx4.par file." <<
                std::endl;
        this->readInputParam((std::string(CLAS12ANA_DIR) + "/Ana/cutFiles/ana_cx4.par").c_str());
    } else if (snx4_runrange && !snx4_runrange_prev) {
        std::cerr << "WARNING:: Run range changed for run " << current_run <<
                ". Setting ana_cx4.par file which is the same as the snx4 cell." << std::endl;
        this->readInputParam((std::string(CLAS12ANA_DIR) + "/Ana/cutFiles/ana_cx4.par").c_str());
    } else if (d_runrange && !d_runrange_prev) {
        std::cerr << "WARNING:: Run range changed for run " << current_run <<
                ". Setting ana_he4.par file. Which is std. liquid cell default." << std::endl;
        this->readInputParam((std::string(CLAS12ANA_DIR) + "/Ana/cutFiles/ana_he4.par").c_str());
...
void clas12ana::Init() {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        ecal_sf_fcn[0][i] = new TF1(Form("ecal_sf_fcn_0_%d", i),
                                    "[0] + [1]/x + [2]/pow(x,2) - [6]*( [3] + [4]/x + [5]/pow(x,2))", 0, 1.5);
        ecal_sf_fcn[1][i] = new TF1(Form("ecal_sf_fcn_1_%d", i),
                                    "[0] + [1]/x + [2]/pow(x,2) + [6]*( [3] + [4]/x + [5]/pow(x,2))", 0, 1.5);

        ecal_p_mean_fcn[i] = new TF1(Form("ecal_p_mean_fcn_%d", i), "[0] + [1]/x + [2]/pow(x,2)", 0, 10);
        ecal_p_fcn[0][i] = new TF1(Form("ecal_p_fcn_0_%d", i),
                                   "[0] + [1]/x + [2]/pow(x,2) - [6]*( [3] + [4]/x + [5]/pow(x,2))", 0, 10);
        ecal_p_fcn[1][i] = new TF1(Form("ecal_p_fcn_1_%d", i),
                                   "[0] + [1]/x + [2]/pow(x,2) + [6]*( [3] + [4]/x + [5]/pow(x,2))", 0, 10);
    }
...
bool clas12ana::DCEdgeCuts(const region_part_ptr &p) {
    ...
    if (p->par()->getCharge() != 0) {
        ...
        // electron DC cuts
        if (p->par()->getCharge() < 0 && (dc_edge_cut_el.size() == 3 && traj_edge_1 > dc_edge_cut_el[0] && traj_edge_2 >
                                          dc_edge_cut_el[1] && traj_edge_3 > dc_edge_cut_el[2]))
            return true;
            // proton DC cuts
        else if (p->par()->getCharge() > 0 && (dc_edge_cut_ptr.size() == 3 && traj_edge_1 > dc_edge_cut_ptr[0] &&
                                               traj_edge_2 > dc_edge_cut_ptr[1] && traj_edge_3 > dc_edge_cut_ptr[2]))
            return true;
        else
            return false;
    } else
...
bool clas12ana::checkGhostTrackCD(const region_part_ptr &p) {
    ...
    */
    ...
    for (auto &p2: protons) {
        // case 1
        if (p->getRegion() == clas12::CD && p2->getRegion() == clas12::CD && p2->sci(CTOF)->getComponent() == p->
            sci(CTOF)->getComponent())
            return true;

            // case 2
        else if (((p->getRegion() == clas12::FD && p2->getRegion() == clas12::CD) || (
                      p->getRegion() == clas12::CD && p2->getRegion() == clas12::FD)) &&
...
double clas12ana::getSF(const region_part_ptr &p) {
    if (p->par()->getPid() == 11)
        return (p->cal(clas12::PCAL)->getEnergy() + p->cal(clas12::ECIN)->getEnergy() + p->cal(clas12::ECOUT)->
                getEnergy()) / p->par()->getP();
    else
        return -9999.;
}
...
bool clas12ana::checkVertexCorrelation(const region_part_ptr &el, const region_part_ptr &p) {
    ...
    if (p->getRegion() == clas12::CD)
        return ((el->par()->getVz() - p->par()->getVz()) > vertex_corr_cuts_cd.at(0) && (
                    el->par()->getVz() - p->par()->getVz()) < vertex_corr_cuts_cd.at(1));
    else if (p->getRegion() == clas12::FD)
        return ((el->par()->getVz() - p->par()->getVz()) > vertex_corr_cuts_fd.at(0) && (
                    el->par()->getVz() - p->par()->getVz()) < vertex_corr_cuts_fd.at(1));
    else
        return true;
}
...
bool clas12ana::checkProtonPidCut(const region_part_ptr &p) {
    ...
    if (itter != pid_cuts_cd.end()) {
        double mom = p->par()->getP();
        // proton_sigma=2;
        double exp_beta = mom / sqrt(pow(mom, 2) + pow(mass_proton, 2));
        // expected beta of particle assuming proton mass
        double tof_diff = (p->getPath() / c) * (1 / p->par()->getBeta() - 1 / exp_beta);
        // TOF difference measured - expected
...
void clas12ana::getLeadRecoilSRC(TLorentzVector beam, TLorentzVector target, TLorentzVector el) {
    ...
    for (int idx_ptr = 0; idx_ptr != protons.size(); ++idx_ptr) {
        ptr.SetXYZM(protons.at(idx_ptr)->par()->getPx(), protons.at(idx_ptr)->par()->getPy(),
                    protons.at(idx_ptr)->par()->getPz(), mass_proton);
