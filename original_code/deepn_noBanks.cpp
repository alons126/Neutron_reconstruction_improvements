...
void Usage() { std::cerr << "Usage: ./code <MC =1,Data = 0> <Ebeam(GeV)> <path/to/ouput.root> <path/to/ouput.pdf> <path/to/cutfile.txt> <path/to/input.hipo> \n"; }
...
int main(int argc, char **argv) {
    ...
    */
    ...
    TH2D *h_diff_ToFc_z_Edep_noNear_goodN_Step1 =
        new TH2D("diff_ToFc_z_Edep_noNear_goodN_Step1", "ToF*c - z [cm] vs. E_{dep} [MeV] of CND Neutrons with no Nearby Tracks;ToF*c-z;E_{dep}", 50, 0, 300, 50, 0, 100);
    hist_list_2.push_back(h_diff_ToFc_z_Edep_noNear_goodN_Step1);
    TH2D *h_diff_ToFc_z_Edep_yesNear_goodN_Step1 =
        new TH2D("diff_ToFc_z_Edep_yesNear_goodN_Step1", "ToF*c - z [cm] vs. E_{dep} [MeV] of CND Neutrons with no Nearby Tracks;ToF*c-z;E_{dep}", 50, 0, 300, 50, 0, 100);
    hist_list_2.push_back(h_diff_ToFc_z_Edep_yesNear_goodN_Step1);
    TH2D *h_diff_ToFc_z_Edep_noNear_badN_Step1 =
        new TH2D("diff_ToFc_z_Edep_noNear_badN_Step1", "ToF*c - z [cm] vs. E_{dep} [MeV] of CND Neutrons with no Nearby Tracks;ToF*c-z;E_{dep}", 50, 0, 300, 50, 0, 100);
    hist_list_2.push_back(h_diff_ToFc_z_Edep_noNear_badN_Step1);
    TH2D *h_diff_ToFc_z_Edep_yesNear_badN_Step1 =
        new TH2D("diff_ToFc_z_Edep_yesNear_badN_Step1", "ToF*c - z [cm] vs. E_{dep} [MeV] of CND Neutrons with no Nearby Tracks;ToF*c-z;E_{dep}", 50, 0, 300, 50, 0, 100);
    ...
    */
    ...
    while (chain.Next() == true) {
        ...
        for (int j = 0; j < allParticles.size(); j++) {
            ...
            if ((theta_nmiss < 40) && (dm_nmiss > -0.5) && (dm_nmiss < 0.5)) { isGN = true; }
            ...
            if (ToF * c - v_hit.Z() < 70)  // TODO: why this cut?
            {
                ...
                for (int k = 0; k < allParticles.size(); k++) {
                    ...
                    // TODO: what is this?
                    int vetoSectorbyLayer[4] = {(allParticles[k]->sci(CTOF)->getComponent() + 1) / 2, allParticles[k]->sci(CND1)->getSector(), allParticles[k]->sci(CND2)->getSector(),
                    ...
                    for (int k = 0; k < 4; k++) {
                        ...
                        if (isPosNear(sdiff, ldiff)) { CNDVeto = true; }
                    }
                ...
                if (isGN) {
                    ...
                    else { h_diff_ToFc_z_Edep_yesNear_goodN_Step1->Fill(ToF * c - v_hit.Z(), edep, weight); }
                } else {
                    if (!CNDVeto)
                        h_diff_ToFc_z_Edep_noNear_badN_Step1->Fill(ToF * c - v_hit.Z(), edep, weight);
                    else { h_diff_ToFc_z_Edep_yesNear_badN_Step1->Fill(ToF * c - v_hit.Z(), edep, weight); }
                }
            }
    ...
    for (int i = 0; i < hist_list_1.size(); i++) { hist_list_1[i]->Write(); }

    for (int i = 0; i < hist_list_2.size(); i++) { hist_list_2[i]->Write(); }
